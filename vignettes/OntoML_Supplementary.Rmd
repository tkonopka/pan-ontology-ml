...
```{r supfig.ontologies.notselected.1, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=supfig.heights[2], fig.cap="\\textbf{Summary of additional ontologies available through the OBO foundry and OLS, but not selected for in-depth analysis (1/2).} Ontologies are summarized (from left to right) by the total number of terms, the depth of terms from the ontology root, number of siblings, number of parents, and number of declared synonymous names. Distributions are displayed as box-plots with whiskers capturing 5\\%-95\\% intervals, boxes 25\\%-75\\% intervals, midlines representing medians, and isolated dots showing the maximal values. The filter code indicates criteria for omitting the ontologies from in-depth analysis: non-recent age ('A'), graph structure ('G')."}
...
```{r supfig.fbdv, eval=TRUE, echo=FALSE, results="asis", fig.width=6.5, fig.height=4.4, fig.cap="\\textbf{Embeddings of fbdv ontology terms}. All panels represent embeddings of fbdv ontology terms into an abstract 2D space. Dots represent ontology terms and are colored by their position in the ontology hierarchy as roots, intermediate nodes, and leaf nodes. \\textbf{(A-C)} UMAP embeddings constructed from semantic distances based on text-annotations. Panels differ in that neighborhoods are computed using \\textbf{(A)} k=5, \\textbf{(B)} k=9, and \\textbf{(C)} k=15 neighbors. Note that the embedding with k=5 is conceptually equivalent to the embedding in one of the primary figures, but differs because of stochasticity. \\textbf{(D-F)} Embeddings constructed from distances based on the ontology graph structure. Panels generated using \\textbf{(D)} a graph layout algorithm, \\textbf{(E)} node2vec with default arguments, and \\textbf{(F)} UMAP with k=15. "}
...
plot_ontology_embedding(fbdv.embeddings$umap.search.k5, fbdv$summary,
						show.labels=FALSE, legend.pos=c(-24, -3),
...
```{r supfig.hp, eval=TRUE, echo=FALSE, fig.width=4, fig.height=1.6, fig.cap="\\textbf{Effect of diffusion on self-retrieval and parent-retrieval performance in the HP ontology.} Both panels use HP ontology term names as queries against models trained with HP name and full-text data. Diffusion is driven by HP FT+ data. Panels show effects on \\textbf{(A)} self-retrieval and \\textbf{(B)} parent-retrieval. Note that panel (B) reproduces one of the panels in the primary figures."}
...
plot.lines(hp.search.diff.summary[difftype=="parents" & obotype=="name"],
		   ...
		   xlab="diffusion strength", ylab="self retrieval, precision",
