...
fig.explaining(perf.model.data, "precision_self",
			   ylab="self retrieval, precision", adj=c(0.2, -1),
...
fig.explaining(perf.model.data, "precision_any_parent",
			   ylab="parent retrieval, precision", adj=c(0.2, -1),
...
```{r fig.casestudies, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=3.4, fig.cap="\\textbf{Feedback from ML for ontology development.} \\textbf{(A)} Unsupervised UMAP embedding of FBDV ontology terms (dots) according to their similarity of text-based annotations. Selected groups of terms are labeled manually. \\textbf{(B)} Parent-retrieval performance in the original FBDV ontology and a modified version with more hierarchical relations. \\textbf{(C)} Effect of diffusion on parent retrieval in the human phenotype ontology, HP. Two lines represent results on models trained with names and FT datasets; all queries performed with names data. \\textbf{(D)} Summary of errors in parent retrieval without (above) and with (below) diffusion. \\textbf{(E)} Example of an ontology term with a large parent-retrieval error, both with and without diffusion. \\textbf{(F)} Activation heatmap for exemplar term representations."}
...
legend(1.1, 0.4, c("name", "FT"), Rcssclass=c("lines"),
	   title="training data")
multipanelLabel(panel.labels[3])
# draw two histograms
