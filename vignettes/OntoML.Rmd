...
```{r fig.all, eval=TRUE, echo=FALSE, fig.width=4.8, fig.height=1.3, fig.cap="\\textbf{Summary of biological ontologies.} All panels show stratification of a collection of ontologies by \\textbf{(A)} the number of terms (classes), \\textbf{(B)} the time since last update, and \\textbf{(C)} the number of their disconnected components. Bars in a dark color convey the criteria used to select ontologies for subsequent analysis."}
...
```{r fig.ontologies, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=6.7, fig.cap="\\textbf{Summary of ontology definitions.} Summary statistics for selected biological ontologies. Vertical panels display the number of terms in each ontology, the distribution of term depths (normalized by the size of each ontology), the distribution of the number of parents for each term, the proportion of terms lacking a text definition, the distribution of number of characters within text definitions, and the distribution of the number of synonyms."}
...
```{r fig.reconstruction, eval=TRUE, echo=FALSE, fig.width=3.0, fig.height=3.8, fig.cap="\\textbf{Reconstruction of ontologies hierarchy.} \\textbf{(A)} Example of text associated with an ontology term from the mammalian phenotype (MP) ontology. One version includes only the term name, another version includes full-text (FT), and another has full text plus definitions of parent terms (FP+). \\textbf{(B)} Graph for a branch of the MP ontology. Dots represent ontology terms, arrows represent directional 'is a' relationships. The dark dot is the root node. \\textbf{(C)} Example of an ontology-reconstruction calculation using k nearest-neighbor search. The first view shows first-ranked predictions; the second view shows the top three predictions. Labels 'hit' and 'miss' indicate when text from a given node is matched to itself via search. \\textbf{(D)} Summary of reconstruction performance averaged over all ontologies."}
...
```{r fig.explaining, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=3.24, fig.cap="\\textbf{Factors affecting empirical ontology reconstruction based on text descriptions.} All panels show relations between precision from machine learning models trained and queried with FT data. Vertical axes show \\textbf{(A)} self-retrieval and \\textbf{(B)} parent-retrieval precision. Selected ontologies are labeled manually. "}
...
plot_ontology_embedding(fbdv.embeddings$plain.data.plain.diff.NA.NA,
						fbdv$summary,
						show.labels=FALSE, legend.pos=c(-29, -5),
...
# misc labeling
polygon(0.5 + c(-7.5,-2.5, 2.5, 7.5, 7.5, 2.5, -2.5, -7.5),
		c(-20, -15, -15, -20, -25, -30, -30, -25),
		Rcssclass="umap")
text(9, -23, "adulthood", Rcssclass=c("embedding", "label"))
polygon(c(20, 20, 28, 28), c(9, 17, 17, 9), Rcssclass="embedding")
lines(c(24, 24), c(3.5, 9), Rcssclass=c("embedding", "label"))
text(14, 1, "larval stages", Rcssclass=c("embedding", "label"))
...
plot.simplebars(rev(.fbdv.bars), at=c(0, 0.1, 0.2, 0.3),
				...
				xlab="parent-retrieval, precision")
multipanelLabel(panel.labels[2])
#
# HP ontology
#
...
plot.lines(hp.search.diff.summary[difftype=="parents" & obotype=="name"],
           ...
           xlab="diffusion strength", ylab="parent retrieval, precision",
...
legend(1.1, 0.4, c("name", "plain"), Rcssclass=c("lines"),
	   title="training data")
multipanelLabel(panel.labels[3])
# draw two histograms
...
multipanelLabel(panel.labels[4])
plot.int.hist(counts.nodiff, main="Search for parent terms",
              ylim=c(0, 10000), xlim=c(-0.5, 12.5), Rcssclass=c("top", "nodiff"))
...
plot.int.hist(counts.diff, main="(with diffusion)",
			  ...
			  show.x.labels=TRUE,
			  ylim=c(0, 10000), xlim=c(-0.5, 12.5), Rcssclass=c("bottom", "diff"))
lines(c(3.5, 3.5, 12.5), c(1200, 2000, 2000), Rcssclass=c("inthist", "label"))
